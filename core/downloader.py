...
class MediaDownloader:
    ...
    async def download(self, url: str, quality: str = "hd", 
        ...
        # Try yt-dlp for all platforms first (most reliable)
        result = await self._download_with_ytdlp(url, quality, platform, progress_callback, download_audio)
        if result.success:
            return result
            
        logger.warning("yt-dlp failed, trying platform-specific fallbacks")
    ...
    async def _download_with_ytdlp(self, url: str, quality: str,
        ...
        try:
            ...
            ydl_opts = {
                ...
                # Use node as JS runtime to solve YouTube bot challenge
                'js_runtimes': {'node': {}},
