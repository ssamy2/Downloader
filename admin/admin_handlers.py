...
def get_admin_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    buttons.append([
        build_btn('STATS',  callback_data="admin:stats")
    ])
    
    # User management
    buttons.append([
        build_btn('BAN_USER',  callback_data="admin:ban"),
        build_btn('UNBAN_ALT',  callback_data="admin:unban")
    ])
    
    # Full access features
    buttons.append([
        build_btn('BCAST_MSG',  callback_data="admin:broadcast"),
        build_btn('RESET_LIMIT',  callback_data="admin:reset_limit")
    ])
    
    # Channel management
    buttons.append([
        build_btn('ADMIN_CHANNELS_MGR',  callback_data="admin:channels")
    ])
    
    # Owner only features
    if is_owner(user_id):
        buttons.append([
            build_btn('ADMIN_ADMINS_MGR',  callback_data="admin:manage_admins")
        ])
    
    buttons.append([
        build_btn('CLOSE',  callback_data="admin:close")
    ])
...
def get_channels_keyboard(channels: List[dict]) -> InlineKeyboardMarkup:
    ...
    buttons.append([
        build_btn('ADD_CHANNEL',  callback_data="add_channel")
    ])
    buttons.append([
        build_btn('BACK',  callback_data="admin:back")
    ])
...
async def ban_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [build_btn('CANCEL',  callback_data="admin:back")]
        ]),
        parse_mode="HTML"
    )
...
async def receive_broadcast_content(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [
                build_btn('CONFIRM',  callback_data="broadcast:confirm"),
                build_btn('CANCEL',  callback_data="broadcast:cancel")
            ]
        ]),
        parse_mode="HTML"
    )
...
async def add_channel_prompt(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [build_btn('CANCEL',  callback_data="admin:channels")]
        ]),
        parse_mode="HTML"
    )
